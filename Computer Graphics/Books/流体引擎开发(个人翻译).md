...
...
    ...
    for (size_t i = 0; i < numberOfEdges; ++i) {
        ...
        if (distance > 0.0) {
            Vector3D force = -stiffness * (distance - restLength) * r.
                rmalized();
...
F_g = mg\tag{1.72}
$$
​		表示作用在流体上的重力，m表示部分流体的质量，以及重力常数。这与我们在质量弹簧的例子中观察到的完全相同(第1.6.2节)，而且就像前面的例子一样，除了重力之外，其他类型的力会累积，然后最终的合力f产生加速度，正如我们已经从牛顿第二运动定律中知道的那样:
